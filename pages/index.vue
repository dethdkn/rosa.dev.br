...
<template>
  <div class="flex flex-wrap items-center justify-center space-x-6 pt-16">
    <div ref="image" class="group relative size-48 cursor-pointer overflow-hidden rounded-full border-4 border-[#F28AA9]" role="button" @click="pinkBlast" @keydown.enter="pinkBlast">
      ...
      <NuxtImg src="/deth.webp" alt="Deth" class="absolute left-0 top-10 size-full -rotate-12 rounded-full object-cover opacity-0 transition-all duration-500 group-hover:top-0 group-hover:rotate-0 group-hover:opacity-100" />
    </div>
    <div class="space-y-2 text-center">
      ...
      <h3 class="text-sm text-[#4C4F69] opacity-0 transition-all duration-500 dark:text-[#CDD5F4]" :class="{'opacity-100': hovered}">
        {{ t('home.drawn_by') }}
      </h3>
    </div>
  </div>
