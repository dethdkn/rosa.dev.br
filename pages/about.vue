...
<template>
	<div class="max-w-7xl mx-auto">
		...
		<div class="fixed top-0 left-0 bg-gray-950 bg-opacity-30 w-full h-full z-[100] transition-all" :class="{ hidden: selected.hidden }" @click="closePopup">
			<Icon v-show="!selected.hidden" name="iconoir:xmark-circle" size="30" class="absolute top-3 right-3 cursor-pointer z-[101] text-[#4C4F69] dark:text-[#CDD5F4] hover:text-[#F28AA9] dark:hover:text-[#F28AA9]" />
			<img id="popup-img" :src="selected.img || '/images/placeholder.png'" :alt="selected.alt" class="absolute top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%] rounded-2xl border-[3px] border-solid object-cover w-[95%] md:w-[950px] cursor-cell border-[#F28AA9]">
		</div>
	</div>
</template>
