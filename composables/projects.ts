export default ref([
	{
		title: 'Agenda de Auditórios',
		badges: [
			{ title: 'Vue.JS', icon: 'vscode-icons:file-type-vue', color: '#41B883' },
			{ title: 'Nuxt.JS', icon: 'vscode-icons:file-type-nuxt', color: '#2DDC82' },
			{ title: 'Nuxt UI', icon: 'vscode-icons:file-type-nuxt', color: '#2DDC82' },
			{ title: 'TailwindCSS', icon: 'devicon:tailwindcss', color: '#38BDF8' },
			{ title: 'LdapJS', icon: 'fxemoji:folder', color: '#59C9FC' },
			{ title: 'Mongoose', icon: 'devicon:mongoose', color: '#840000' },
		],
		urls: [
			{ url: 'https://auditorios.cbpf.br', external: true, icon: 'iconoir:open-new-window' },
			{ url: 'https://youtu.be/14zR8I2AYcM', external: true, icon: 'iconoir:media-video' },
			{ url: 'https://github.com/dethdkn/agenda-de-auditorios', external: true, icon: 'iconoir:github-circle' },
		],
		descriptionKey: 'projects.agenda_auditorios_description',
	},
	{
		title: 'ID CBPF',
		badges: [
			{ title: 'Vue.JS', icon: 'vscode-icons:file-type-vue', color: '#41B883' },
			{ title: 'Nuxt.JS', icon: 'vscode-icons:file-type-nuxt', color: '#2DDC82' },
			{ title: 'Nuxt UI', icon: 'vscode-icons:file-type-nuxt', color: '#2DDC82' },
			{ title: 'TailwindCSS', icon: 'devicon:tailwindcss', color: '#38BDF8' },
			{ title: 'LdapJS', icon: 'fxemoji:folder', color: '#59C9FC' },
			{ title: 'Mongoose', icon: 'devicon:mongoose', color: '#840000' },
		],
		urls: [
			{ url: 'https://youtu.be/Zq7PwGxcsLg', external: true, icon: 'iconoir:media-video' },
			{ url: 'https://github.com/dethdkn/idcbpf', external: true, icon: 'iconoir:github-circle' },
		],
		descriptionKey: 'projects.idcbpf_description',
	},
	{
		title: 'Ldap-Sha512',
		badges: [
			{ title: 'NodeJS', icon: 'vscode-icons:file-type-node', color: '#83CD29' },
			{ title: 'Typescript', icon: 'devicon:typescript', color: '#137ACC' },
			{ title: 'Vitest', icon: 'devicon:vitest', color: '#FCC72B' },
		],
		urls: [
			{ url: 'https://www.npmjs.com/package/ldap-sha512', external: true, icon: 'iconoir:npm' },
			{ url: 'https://github.com/dethdkn/ldap-sha512', external: true, icon: 'iconoir:github-circle' },
		],
		descriptionKey: 'projects.ldapsha512_description',
	},
	{
		title: 'karinebarbosa.com',
		badges: [
			{ title: 'Bootstrap', icon: 'devicon:bootstrap', color: '#8419FA' },
			{ title: 'Jquery', icon: 'devicon:jquery', color: '#0E68AB' },
		],
		urls: [
			{ url: 'https://karinebarbosa.com', external: true, icon: 'iconoir:open-new-window' },
		],
		descriptionKey: 'projects.karine_description',
	},
	{
		title: 'rosa.dev.br',
		badges: [
			{ title: 'Vue.JS', icon: 'vscode-icons:file-type-vue', color: '#41B883' },
			{ title: 'Nuxt.JS', icon: 'vscode-icons:file-type-nuxt', color: '#2DDC82' },
			{ title: 'TailwindCSS', icon: 'devicon:tailwindcss', color: '#38BDF8' },
		],
		urls: [
			{ url: '/', external: false, icon: 'iconoir:open-new-window' },
			{ url: 'https://github.com/dethdkn/rosa.dev.br', external: true, icon: 'iconoir:github-circle' },
		],
		descriptionKey: 'projects.rosa_description',
	},
])
