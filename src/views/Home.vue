...
<template>
    <section class="min-h-[calc(100vh-2rem)] flex items-center justify-center relative cursor-none overflow-hidden">
        <div class="absolute bg-cyan-400/50 dark:bg-sky-600/30 rounded-full -translate-x-[75%] -translate-y-1/2 blur-2xl dark:blur-3xl" :style="{left: `${x}px`, top: `${y}px`, width: `${size}px`, height: `${size}px`, opacity}"/>
        <h2 ref="logo" class="font-bold text-center text-5xl dark:text-white transition-500" :style="{maskImage: logoGradient}">
            <p>{{ t('home.title') }}</p>
